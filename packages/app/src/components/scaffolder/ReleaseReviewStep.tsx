...
/** Extract KEY names from KEY=VALUE text */
function extractKeys(text: string | undefined): string[] {
  if (!text) return [];
  return text
    .split('\n')
    .filter(l => l.trim())
    .map(line => {
      ...
      return eqIndex > 0 ? line.substring(0, eqIndex).trim() : line.trim();
    });
}
...
/** Detect if this is the Onboard Service template by checking form data fields */
...
export function ReleaseReviewStep(props: ReviewStepProps) {
  ...
  const envKeys = extractKeys(formData.envVars as string);
  const envCount = envKeys.length;
  const secretKeys = extractKeys(formData.secretEnvVars as string);
  const secretCount = secretKeys.length;
  const maskedSecrets = maskSecrets(formData.secretEnvVars as string);
  const provisionDb = Boolean(formData.provisionDatabase);
  const dbInjectedVars = ['DATABASE_URL', 'DB_HOST', 'DB_PORT', 'DB_NAME', 'DB_USER', 'DB_PASSWORD'];
          ...
          <ReviewSection title="Configuration">
            <Table size="small" className={classes.table}>
              <TableBody>
                ...
                {provisionDb && (
                  <TableRow>
                    <TableCell>Database (auto-injected)</TableCell>
                    <TableCell>
                      <Box>
                        <Chip
                          icon={<StorageIcon />}
                        ...
                        />
                        <Box mt={0.5}>
                          {dbInjectedVars.map((v, i) => (
                            <Typography
                              key={i}
                              variant="body2"
                              className={classes.maskedValue}
                            >
                              {v}={v === 'DATABASE_URL' ? 'postgresql://…' : '****'}
                            </Typography>
                          ))}
                        </Box>
                      </Box>
      ...
      <div className={classes.previewBlock}>
        ...
        {provisionDb && (
          <div className={classes.previewItem}>
            <CheckCircleOutlineIcon />
            <Typography variant="body2">
              Provision PostgreSQL database &amp; inject <strong>DATABASE_URL</strong> + credentials
            </Typography>
          </div>
        )}
        {hasHost && (

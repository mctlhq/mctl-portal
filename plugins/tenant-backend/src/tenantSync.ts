...
export class TenantSync {
  ...
  private async fetchTenant(dirName: string, syncedAt: string, githubToken: string): Promise<{ tenant: Tenant; members: Array<{ userId: string; role: string }> } | null> {
    ...
    const tenant: Tenant = {
      ...
      quotas: {
        'requests.cpu': t.quotas?.['requests.cpu'] ?? '4',
        'requests.memory': t.quotas?.['requests.memory'] ?? '8Gi',
        'limits.cpu': t.quotas?.['limits.cpu'] ?? '8',
        'limits.memory': t.quotas?.['limits.memory'] ?? '16Gi',
        pods: t.quotas?.pods ?? '20',
